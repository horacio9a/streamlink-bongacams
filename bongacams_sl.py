# Bongacams Streamlink Remote 24/7 Plugin v.1.0.4 by @horacio9a for Python 2.7.16 - Credits also to @sdfwv
# coding: utf-8
        ...
        if hls_url:
         try:
          for s in HLSStream.parse_variant_playlist(self.session, hls_url, headers=headers).items():
           ...
           print (colored('\n => SL-24/7-REC => {} <=','yellow','on_red')).format(fn)
           print
           command = ('{} hls://"{}" best -Q -o "{}"'.format(streamlink,hls_url,pf))
           os.system(command)
