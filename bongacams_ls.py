# Bongacams Streamlink LIVESTREAMER Remote 24/7 Plugin v.1.0.4 by @horacio9a for Python 2.7.16 - Credits also to @sdfwv
# coding: utf-8
        ...
        if hls_url:
         try:
          for s in HLSStream.parse_variant_playlist(self.session, hls_url, headers=headers).items():
           ...
           fn = real_name + '_BC_' + timestamp + '.mp4'
           pf = (path + fn)
           livestreamer = config.get('files', 'livestreamer')
           print (colored('\n => LS-24/7-REC => {} <=','yellow','on_red')).format(fn)
           print
           command = ('{} hlsvariant://"{}" best -Q -o "{}"'.format(livestreamer,hls_url,pf))
           os.system(command)
